...
@section('content')
                        ...
                        <div class="row">
                            <div class="col-12 col-lg-6 px-5 pt-5">
                                ...
                                <x-admin.input type="text" placeholder="No Transaksi" label="No" name="noTransaksi"
                                    id="noTransaksi" value="{{ $kodeTransaksi }}" readonly />
                                    ...
                                    <table class="text-sm fw-bold">
                                        <tr>
                                            ...
                                            <td class="px-5 d-flex gap-2">Rp. <input type="number"
                                                    class="form-control form-control-sm" name="subTotal" readonly></td>
                                        </tr>
                                        <tr>
                                            <td>Diskon</td>
    ...
    {{-- <script>
        document.getElementById('tanggal').addEventListener('change', function() {
            ...
            // Format tanggal ke YYYYMM
            var year = selectedDate.getFullYear();
            var month = ('0' + (selectedDate.getMonth() + 1)).slice(-2);

            // Buat variabel untuk menyimpan kode transaksi
            var kodeTransaksi = '';

            // Ambil nomor urut terakhir dari server melalui API atau simpan nomor urut terakhir dalam variabel JS
            var lastNumber = 1; // Asumsikan nomor urut terakhir adalah 1, ubah sesuai dengan kebutuhan Anda

            // Jika bulan atau tahun berubah, reset nomor urut
            // Misalnya Anda bisa menyimpan nomor urut terakhir dalam variabel JS atau melakukan panggilan ke server untuk mendapatkan nomor urut terbaru

            // Format nomor transaksi baru
            kodeTransaksi = `${year}${month}-${('0000' + lastNumber).slice(-4)}`;

            // Update input No Transaksi
            document.getElementById('noTransaksi').value = kodeTransaksi;
